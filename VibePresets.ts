...
export const PRESET_CATEGORIES: { id: PresetCategory; label: string; icon: string; desc: string }[] = [
...
// ─── Preset option (dropdown / slider value) ─────────────────────────────────

export interface PresetOption {
  key:     string;
  label:   string;
  type:    'select' | 'slider' | 'toggle' | 'color';
  /** Default value index (select), default number (slider/color), default bool (toggle) */
  default: unknown;
  /** For select: choices. For slider: [min, max, step]. */
  choices?: string[];
...
export interface Preset {
  ...
  subtitle:    string;
  icon:        string;
  /** CSS gradient or solid for the thumbnail swatch */
  thumbGrad:   string;
  tags:        string[];
  /** Tunable options shown in the right "tweak" panel */
  options:     PresetOption[];
  /**
   * Engine config produced when this preset is applied.
   * Keys map to Pyrite64 component / scene / render settings.
   */
  config:      Record<string, unknown>;
  /**
   * Prompt injected into the vibe chat when the user clicks
   * "Refine with Vibe Coder" — lets AI iterate on this preset.
   */
  prompt:      string;
}
...
const SHADING_PRESETS: Preset[] = [
  ...
  {
    id: 'shade-anime', category: 'shading',
    title: 'Anime', subtitle: 'Soft bands + rim lighting + vivid colors',
    icon: '⛩', thumbGrad: 'linear-gradient(135deg, #6c3aad 0%, #ff6eb4 100%)',
    tags: ['anime', 'manga', 'rim-light'],
    options: [
      { key: 'bandCount', label: 'Shade Bands', type: 'slider', default: 3,
        range: [2, 5, 1] },
      { key: 'rimStrength', label: 'Rim Light', type: 'slider', default: 0.6,
        range: [0, 1, 0.05] },
      { key: 'saturation', label: 'Saturation', type: 'slider', default: 1.3,
        range: [0.5, 2, 0.1] },
      { key: 'outlineColor', label: 'Outline Color', type: 'color', default: '#1a0d26' },
    ],
    config: {
      renderPipeline: 'default',
      cartoonStyle: 'anime',
      vertexEffect: 'CELSHADE_COLOR',
      outlineEnabled: true, outlineMode: 0, outlineThickness: 1.5,
      celBands: 3, rimStr: 0.6, saturation: 1.3,
    },
    prompt: 'I chose the Anime shading preset — soft gradients with rim lighting and vivid colors. Help me tune the rim light color per character and set up anime-style eye highlights.',
  },
  {
    id: 'shade-comic', category: 'shading',
    title: 'Comic Book', subtitle: 'Bold outlines + halftone dots',
    icon: '💥', thumbGrad: 'linear-gradient(135deg, #ff4444 0%, #ffdd00 50%, #4488ff 100%)',
    tags: ['comic', 'halftone', 'pop-art'],
    options: [
      { key: 'halftoneStr', label: 'Halftone Strength', type: 'slider', default: 0.7,
        range: [0, 1, 0.05] },
      { key: 'halftoneScale', label: 'Dot Size', type: 'slider', default: 3.0,
        range: [2, 8, 0.5] },
      { key: 'outlineThick', label: 'Outline Thickness', type: 'slider', default: 3.0,
        range: [1, 6, 0.25] },
      { key: 'saturation', label: 'Saturation', type: 'slider', default: 1.4,
        range: [0.5, 2, 0.1] },
    ],
    config: {
      renderPipeline: 'default',
      cartoonStyle: 'comic-book',
      vertexEffect: 'CELSHADE_COLOR',
      outlineEnabled: true, outlineMode: 0, outlineThickness: 3.0,
      celBands: 3, halftoneStr: 0.7, saturation: 1.4,
    },
    prompt: 'I chose the Comic Book shading preset — bold black outlines with halftone dot shadows. Help me set up primary-color materials and action-line effects for a classic comic look.',
  },
  {
    id: 'shade-watercolor', category: 'shading',
    title: 'Watercolor', subtitle: 'Soft edges + paper texture + muted palette',
    icon: '🖌', thumbGrad: 'linear-gradient(135deg, #b8d4e3 0%, #f5e6d3 100%)',
    tags: ['watercolor', 'painterly', 'soft'],
    options: [
      { key: 'paperStr', label: 'Paper Texture', type: 'slider', default: 0.5,
        range: [0, 1, 0.05] },
      { key: 'saturation', label: 'Saturation', type: 'slider', default: 0.75,
        range: [0.3, 1.5, 0.05] },
      { key: 'outlineStr', label: 'Edge Softness', type: 'slider', default: 0.3,
        range: [0, 0.8, 0.05] },
      { key: 'warmth', label: 'Warmth', type: 'slider', default: 0.15,
        range: [-0.5, 0.5, 0.05] },
    ],
    config: {
      renderPipeline: 'default',
      cartoonStyle: 'watercolor',
      vertexEffect: 'CELSHADE_COLOR',
      outlineEnabled: true, outlineMode: 0, outlineThickness: 1.0,
      celBands: 6, paperStr: 0.5, saturation: 0.75,
    },
    prompt: 'I chose the Watercolor shading preset — soft painterly edges with paper texture and muted colors. Help me set up gentle lighting and pastel materials for an illustrated storybook look.',
  },
  {
    id: 'shade-retro-cartoon', category: 'shading',
    title: 'Retro Cartoon', subtitle: 'Warm tones + cross-hatch shadows',
    icon: '📺', thumbGrad: 'linear-gradient(135deg, #8b4513 0%, #f4a460 50%, #ffdab9 100%)',
    tags: ['retro', 'vintage', 'hatching'],
    options: [
      { key: 'hatchStr', label: 'Hatch Strength', type: 'slider', default: 0.5,
        range: [0, 1, 0.05] },
      { key: 'hatchDensity', label: 'Hatch Density', type: 'slider', default: 80,
        range: [30, 150, 5] },
      { key: 'warmth', label: 'Warmth', type: 'slider', default: 0.3,
        range: [-0.3, 0.5, 0.05] },
      { key: 'outlineColor', label: 'Outline Color', type: 'color', default: '#261400' },
    ],
    config: {
      renderPipeline: 'default',
      cartoonStyle: 'retro',
...
export function getPresetsByCategory(cat: PresetCategory): Preset[] {
...
export function getAllTags(): string[] {
  ...
  for (const p of ALL_PRESETS) p.tags.forEach(t => tags.add(t));
  return [...tags].sort();
}

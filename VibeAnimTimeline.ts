...
export interface Track {
  id:       string;
  label:    string;       // e.g. "pos.x", "rot.y", "clip"
  type:     'number' | 'clip';
  color:    string;       // CSS color for keyframe diamonds
  keyframes: Keyframe[];
}
...
export interface TimelineState {
  ...
  playheadTime:  number;   // current scrub position
  playing:       boolean;
  looping:       boolean;
  zoom:          number;   // pixels per second
  scrollX:       number;   // horizontal scroll offset in px
...
export interface TimelineEvents {
  ...
  keyframeSelect:  (ids: string[]) => void;
  keyframeMove:    (id: string, newTime: number) => void;
  keyframeAdd:     (trackId: string, time: number, value: number | string) => void;
  keyframeDelete:  (ids: string[]) => void;
  export:          (tracks: Track[]) => void;
}
...
export class VibeAnimTimeline {
  ...
  on<K extends TimelineEventType>(event: K, cb: TimelineEvents[K]): this {
  ...
  private buildControls(): HTMLElement {
    ...
    // Store refs
    (this as any)._playBtn = playBtn;
    (this as any)._timeLabel = timeLabel;
  ...
  private play(): void {
    ...
    (this as any)._playBtn.textContent = '⏸';
  ...
  private toggleLoop(btn: HTMLButtonElement): void {
  ...
  private updateTimeLabel(): void {
    ...
    const label = (this as any)._timeLabel as HTMLElement;
    if (label) label.textContent = `${sec}:${String(frac).padStart(2, '0')}`;
  }
  ...
  private draw(): void {
    ...
    for (let t = Math.floor(startTime / TICK_MINOR_INTERVAL) * TICK_MINOR_INTERVAL;
         t <= endTime; t += TICK_MINOR_INTERVAL) {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      for (const kf of tracks[i].keyframes) {
        ...
        if (kx < LABEL_WIDTH - KEYFRAME_SIZE || kx > W + KEYFRAME_SIZE) continue;
  ...
  private canvasToTime(clientX: number): number {
  ...
  private hitTestKeyframe(clientX: number, clientY: number): { trackIdx: number; kfId: string } | null {
    const rect = this.canvas.getBoundingClientRect();
    ...
    for (let i = 0; i < this.state.tracks.length; i++) {
      ...
      if (Math.abs(my - ty) > KEYFRAME_SIZE) continue;
  ...
  private onDoubleClick(e: MouseEvent): void {
    ...
    if (trackIdx < 0 || trackIdx >= this.state.tracks.length) return;
  ...
  private emit(event: string, ...args: any[]): void {
    for (const cb of this.listeners[event as TimelineEventType] ?? []) {
      (cb as any)(...args);
    }
  }
}

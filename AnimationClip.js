...
export function createClip(name, duration = 1.0) {
    return { name, duration, loop: false, tracks: [] };
}
/** Add a track to a clip. Returns the new track. */
export function addTrack(clip, targetNode, property) {
    const track = { targetNode, property, keyframes: [] };
    clip.tracks.push(track);
    return track;
}
/** Insert a keyframe into a track, keeping keyframes sorted by time. */
export function insertKeyframe(track, kf) {
 ...
 */
export function evaluateTrack(track, time) {
...
export function deserializeClip(json) {
    const obj = JSON.parse(json);
    validateClip(obj);
    return obj;
}
/** Validate that a parsed object conforms to the AnimClip schema. */
export function validateClip(obj) {
    ...
    const validProps = ['position', 'rotation', 'scale'];
    const validEasings = ['linear', 'step', 'bezier'];
    for (const track of clip.tracks) {
        ...
        if (!validProps.includes(t.property)) {
        ...
        for (const kf of t.keyframes) {
            ...
            if (!validEasings.includes(k.easing)) {

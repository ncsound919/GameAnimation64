 ...
 */
export function exportClipToN64(clip) {
    const warnings = [];
    ...
    if (unclampedFrameCount > MAX_FRAMES) {
        warnings.push(`Clip "${clip.name}" clamped to ${MAX_FRAMES} frames (${(MAX_FRAMES / SAMPLE_RATE_FPS).toFixed(1)}s). ` +
    ...
    // Bake each track
    const bakedTracks = clip.tracks.map(track => bakeTrack(track, frameCount, clip.duration));
    // Calculate size
...
function bakeTrack(track, frameCount, duration) {
    const samples = [];
    for (let f = 0; f < frameCount; f++) {
        ...
        if (track.property === 'rotation') {
            ...
            samples.push(Math.round((((val[0] % 360) + 360) % 360) * ROTATION_SCALE), Math.round((((val[1] % 360) + 360) % 360) * ROTATION_SCALE), Math.round((((val[2] % 360) + 360) % 360) * ROTATION_SCALE));
        }
        else {
            // Position/scale → 16.16 fixed-point
            samples.push(Math.round(val[0] * FIXED_POINT_SCALE), Math.round(val[1] * FIXED_POINT_SCALE), Math.round(val[2] * FIXED_POINT_SCALE));
        }
    }
    return { targetNode: track.targetNode, property: track.property, samples };
}
function generateHeader(id, clip, tracks, frameCount) {
    ...
    for (let ti = 0; ti < tracks.length; ti++) {
        ...
        const comment = bt.property === 'rotation'
            ? '/* 0..65535 → 0..360° */'
            : '/* 16.16 fixed-point */';
        lines.push(`${comment}`);
        lines.push(`static const int32_t ${sanitizeIdentifier(arrayName)}[${frameCount}][3] = {`);
        for (let f = 0; f < frameCount; f++) {
            const base = f * 3;
